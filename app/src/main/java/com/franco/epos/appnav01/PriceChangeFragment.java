  ...
  //  private DatabaseHelper db;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
       ...
       // actionType = getIntent().getStringExtra("ActType");
        ...
        btnPckQty = view.findViewById(R.id.btn_pckQty);
...
//                System.out.println("pack quantity action type : "+ actionType + dictVal.get("itemLU").toString());
    ...
    private String setSerItemDataAdapter(final String type) {
        ...
        dict = selectArray("itemProduct","select id, ItemLookup, description, barCode, price, avaiable, vat, Rprice, Rcost, department, category from itemproduct where barCode = '"+itemSearch.getText().toString().trim()+"' OR ItemLookup = '"+itemSearch.getText().toString().trim()+"'");
    ...
    private Dictionary selectArray(String type, String query) {
        ...
        String rrPrice = "", rrCost = "", itemVat = "", itemDep = "", itemCat = "";
        ...
        if (stmt != null) try {
            ...
            //SampleArrayList.clear();vat, Rprice, Rcost
            dict.isEmpty();
            while (rs.next()) {
                ...
                rrPrice = rs.getString("Rprice");
                rrCost = rs.getString("Rcost");
                itemVat = rs.getString("vat");
                itemDep = rs.getString("department");
                itemCat = rs.getString("category");
            ...
            ResultSet rsd = stmt.executeQuery("select description, groups, ageCheck, commission, vat from department where depart_id = '"+itemDep+"'");
            ...
            ResultSet rsc = stmt.executeQuery("select category, vat from category where id = '"+itemCat+"'");

            while (rsc.next()) {

                catDescr = rsc.getString("category");
                catVat = rsc.getString("vat");

            }

            System.out.println("Category details : "  + catDescr + catVat);
        ...
        dict.put("rrPrice", rrPrice);
        dict.put("rrCost", rrCost);
        dict.put("itemVat", itemVat);
        dict.put("depDescr", depDescr);
        dict.put("depGroups", depGroups);
        dict.put("depAgeCheck", depAgeCheck);
        dict.put("depCommission", depCommission);
        dict.put("depVat", depVat);
        dict.put("catDescr", catDescr);
        dict.put("catVat", catVat);
                ...
                if(resultProd == 1) {
                    ...
                    if (Rprice != trackPrice){
                        ...
                        if(resultPriceN == 1) {
                            ...
                            itemUploadTB = new ItemUploadTB(1,"pricechange",dictVal.get("itemLU").toString().trim(),dictVal.get("descr").toString().trim(),dictVal.get("barCode").toString().trim(),dictVal.get("itemVat").toString().trim(),dictVal.get("rrPrice").toString().trim(),dictVal.get("rrCost").toString().trim(),dictVal.get("depDescr").toString().trim(),dictVal.get("depGroups").toString().trim(),dictVal.get("depAgeCheck").toString().trim(),dictVal.get("depCommission").toString().trim(),dictVal.get("depVat").toString().trim(),dictVal.get("catDescr").toString().trim(),dictVal.get("catVat").toString().trim());
                            ...
                            if (itemsP.size() > 0){
                                for (int j=0; j<itemsP.size();j++){
                                    ...
                                    *//*Item item = new Item();
                                    ...
                                    items.add(item);*//*
                                    System.out.println("Inserted details details : "  + itemsP.get(j).getId() + itemsP.get(j).getItem_lookup() + itemsP.get(j).getDep_descr() + itemsP.get(j).getCost());
        ...
        if ((requestCode == 10001) && (resultCode == Activity.RESULT_OK)){
            ...
            if (data.getStringExtra("NavValue").equals("QuickAdd")) {
               ...
               // setSerItemDataAdapter(actionType);
                ...
                itemInPrice.setText(data.getStringExtra("itemPckPUpdVal"));
